...
public extension Sextant {
    @inlinable func query<A>(_ root: JsonAny, value pathArray: [Hitch]) -> (A?)? {
    ...
    @inlinable func query<A>(_ root: String, value pathArray: [Hitch]) -> (A?)? {
        guard let values: JsonArray = query(root, values: pathArray) else { return nil }
        return (values.get(0) as? A)
    }
    ...
    @inlinable func query<A>(_ root: Hitch, value pathArray: [Hitch]) -> (A?)? {
        guard let values: JsonArray = query(root, values: pathArray) else { return nil }
        return (values.get(0) as? A)
    }
    ...
    @inlinable func query<A>(_ root: Data, value pathArray: [Hitch]) -> (A?)? {
        guard let values: JsonArray = query(root, values: pathArray) else { return nil }
        return (values.get(0) as? A)
    }
